...
``:record-struct-name-prefix:`` (optional)
   ...
   ``Record``.  Available for Go, Java, Kotlin, Python, Rust, and
...
``:heterogeneous-strategy:`` (optional, defaults to ``auto``)
   ...
   ``record``
      ...
      names can be customised with ``:record-struct-name-prefix:`` and
      ``:record-shape-names:``.  Available for Go, Java, Kotlin,
